...
<html lang="en">
    ...
    <div class="row" style="min-height: 10px;margin-bottom: 20px">
      ...
      <p class="sous_titre1" style="font-size: 18px;">Clicker sur une catégorie pour sélectionner les produits voulus, remplir la quantité et ajouter au panier puis appuyer sur passer commande:</p>
      <?= $this->session->flashdata('message') ?>
      <?php
      // echo 5%4;
      $categori=$this->Model->getList('categorie');
      foreach ($categori as $value) {
        ...
        ?>
        <img class="img1 categ" id="<?=$value['CATEGORIE_ID']?>" src="<?=base_url()?><?=$value['CATEGORIE_IMAGE']?>" width="100%" heigth=""><b><article style="font-size: 12px"><?=$value['CATEGORIE_DESCR']?></article></b></div>
...
<p><a class="primary-btn cta-btn form-control " href="<?=base_url("Commande_sur_caisse/Confirmation_commande")?>">verifier le pannier</a>
...
<script type="text/javascript">
  ...
  var readMoreHtml=$(this).html();
      var lessText=readMoreHtml.substr(0,15);

      if(readMoreHtml.length >15){
...
$('.categ').click(function(){
  // alert();
    ...
    $.ajax({
                            url:"<?php echo base_url() ?>Commande_sur_caisse/produit",
                            method:"POST",
                            //async:false,
                            data: {id:id},
