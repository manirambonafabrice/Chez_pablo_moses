                  ...
                  <div>
                        <div>
                            <table class="table table-condensed">
                                <thead>
                                <tr>
                                    <td class="text-center" style="font-size: 10px;text-align: left;"><strong>No</strong></td>
...
$total2=0;
 foreach ($this->cart->contents() as $key => $items) {
        if(preg_match("/SOC/", $items['soc'])){
          ...
          ?>
                                <tr>
                                    <td class="text-center"  style="font-size: 10px;text-align: left;">
                                        <?=$items["qty"]?>
                                    </td >
                                    <td class="text-center"  style="font-size: 10px;text-align: left;">
                                        <?=$items["NOM_PRODUIT"]?>
                                    </td >
                                    <td class="text-center" style="font-size: 10px;text-align: right;">
                                           <?=$items["PV"]?>
                                    </td >
                                    <td class="text-center" style="font-size: 10px;text-align: right;">
                                           <?=$total1?>
                               ...
                               <select id="USER_ID" name="USER_ID" class="form-control selectpicker"  data-live-search="true" required>
                                ...
                                <option value="-1">--AUCUN--</option>
                                 ...
                                 foreach ($users as $value) {
                                   ?>
                                    <option value="<?=$value['USER_ID']?>"><?=$value['NOM_USER']?> <?=$value['PRENOM_USER']?>(<?=$value['USERNAME']?>: <?=$value['TELEPHONE']?>)</option>
                                   <?php
                                 }
                                 ?>
    ...
    $.ajax({
                            ...
                            success:function(data)
                                               ...
                                               if (data!='non') {
                                                ...
                                                //$('#COMMUNE').selectpicker('refresh'); 
                                                }else{alert('CETTE QUANTITE N\'EST PAS DISPONIBLE')}                                                  
                                                 }
                });

    }else{
    alert("Veuillez Choisir le client, ou choisir AUCUN");
 }
