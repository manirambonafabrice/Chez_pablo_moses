...
class Commande_sur_caisse extends CI_Controller {
...
$data1=array('upload_data'=>$this->upload_pieceJointe->data());
$id_actualite=$this->Model->insert_last_id('produit',array('PRODUIT_DESCR'=>$titre,'PRIX_ACHAT'=>$prix_achat,'PRODUIT_PRIX'=>$prix,'CATEGORIE_ID'=>$categori,'PRODUIT_IMAGE'=>"uploads/produits/".$data1['upload_data']['file_name']));
			 $data['message']='<div class="alert alert-success text-center"> La catégorie est enregistrée avec succès</div>';
  $data['message']='<div class="alert alert-success text-center">Enregistrement avec succès</div>';
...
<div class="container">
		<div class="row" style="min-height: 10px;margin-bottom: 20px;margin-top:20px ">
			...
			<div class='modal fade' id='produit' tabindex="-1">
				...
				<input type="number" name="prix_achat" class="form-control input-sm" autocomplete="off" required>	
			...
			?>
					...
					<ul class="nav nav-tabs">
					    ...
					    $j=1;
					    $x=1;
							$produit=$this->Model->getList('produit',array("CATEGORIE_ID"=>$id));
...
<script type="text/javascript">
 ...
 public function Confirmation_commande(){
  $this->load->view('confirmation_commande_sur_caisse_view');
}
